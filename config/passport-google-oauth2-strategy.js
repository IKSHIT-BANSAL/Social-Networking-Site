...
passport.use(new googleStrategy({
    ...
    //find a user 
    User.findOne({email:profile.emails[0].value}).exec(function(err,user){      //profile.email[] is array so we fetch its 1 value
        if(err){
        ...
        if(user){
            ...
            User.create({
                ...
                password:crypto.randomBytes(20).toString('hex')   //this how crypto works fetching 20 bytes and string in hexadecimal
            },function(err,user){
                if(err){
